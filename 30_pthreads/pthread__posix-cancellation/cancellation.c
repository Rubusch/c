...
int main()
{
	...
	for (idx = 0; idx < NUM_THREADS; ++idx) {
		pthread_create(&threads[idx], NULL, search,
			       (void *)(intptr_t)pid);
	}
...
void *search(void *arg)
{
  ...
  */
	...
	/*
     push the cleanup routine (print_it) onto the thread cleanup stack. This
     routine will be called when the thread is cancelled.  Also note that the
     pthread_cleanup_push call must have a matching pthread_cleanup_pop call.
     The push and pop calls MUST be at the same lexical level within the code
  */

	/*
     pass address of 'idx' since the current value of 'idx' is not the one we
