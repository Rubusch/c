...
NOTE(MUTEX_PROTECTS_DATA(
	stat_lk,
	st_start st_dir_search st_file_search st_line_search st_cascade
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "d:e:bchilnsvwruf:p:BCSZzHP:")) != EOF) {
		...
		case 'e':
			if (regexp_cnt == MAXREGEXP) {
				fprintf(stderr,
					"Max number of regexp's (%d) exceeded!\n",
					MAXREGEXP);
				exit(1);
			}
			flags |= FE_REGEXP;
			if ((string[regexp_cnt] = (char *)malloc(
				     strlen(optarg) + 1)) == NULL) {
				fprintf(stderr,
					"tgrep: No space for search string(s)\n");
				exit(1);
			}
	...
	if (!(flags & FE_REGEXP)) {
		...
		if ((string[0] = (char *)malloc(strlen(argv[optind]) + 1)) ==
		    NULL) {
			fprintf(stderr,
				"tgrep: No space for search string(s)\n");
			exit(1);
  ...
  */
	...
	STAT_AGAIN:
		...
		case S_IFREG:
			if (flags & TG_FILEPAT) {
				if (pmatch(pm_file_pat, argv[optind],
					   &pm_file_len))
    ...
    */
	...
	if (flags & FX_STDIN) {
		fprintf(stderr,
			"tgrep: stdin option is not coded at this time\n");
	...
	while (1) {
		mutex_lock(&work_q_lk);
		while ((work_q == NULL || current_open_files == 0 ||
			tglimit <= 0) &&
		       all_done == 0) {
			if (flags & FS_STATS) {
				mutex_lock(&stat_lk);
		...
		case DIRT:
			...
			if (cascade_pool_cnt) {
				...
				DP(DLEVEL2,
				   ("Sent work to cascade pool thread\n"));
			} else {
				mutex_unlock(&cascade_q_lk);
				err = thr_create(NULL, 0, cascade, (void *)work,
						 ...
						 &tid);
				DP(DLEVEL2,
				   ("Sent work to new cascade thread\n"));
				thr_setprio(tid,
					    64); /* set cascade to middle */
				if (flags & FS_STATS) {
					mutex_lock(&stat_lk);
					st_cascade++;
					mutex_unlock(&stat_lk);
		...
		case FILET:
			...
			if (search_pool_cnt) {
				...
				DP(DLEVEL2,
				   ("Sent work to search pool thread\n"));
			} else {
				mutex_unlock(&search_q_lk);
				err = thr_create(
					NULL, 0, search_thr, (void *)work,
					THR_DETACHED | THR_DAEMON | THR_NEW_LWP,
					&tid);
				thr_setprio(tid, 0); /* set search to low */
				DP(DLEVEL2,
				   ("Sent work to new search thread\n"));
				if (flags & FS_STATS) {
					mutex_lock(&stat_lk);
					st_search++;
					mutex_unlock(&stat_lk);
		...
		default:
			fprintf(stderr,
				"tgrep: Internal error, work_t->tp no valid\n");
			exit(1);
		}
      ...
      */
            ...
            */
				...
				if (bm_pmatch(bm_pat, line)) {
					if (!(flags & FV_REVERSE)) {
						add_output_local(&out, wt,
								 line_count,
								 byte_count,
								 rline);
						continue_line(rline, fin, out,
							      wt, &line_count,
							      &byte_count);
					} else {
						show_line = 0;
...
void *cascade(void *arg) /* work_t *arg */
{
	...
	while (1) {
		...
		if ((dp = opendir(fpath)) == NULL) {
			if (!(flags & FS_NOERROR))
				fprintf(stderr,
					"tgrep: Can't open dir %s, %s. Ignored.\n",
					fpath, strerror(errno));
			goto DONE;
		}
		while ((readdir_r(dp, dent)) != NULL) {
			restart_cnt =
				10; /* only try to restart the interupted 10 X */
		...
		RESTART_STAT:
			...
			case S_IFREG:
				if (flags & TG_FILEPAT) {
					if (pmatch(pm_file_pat, dent->d_name,
						   &pm_file_len)) {
						DP(DLEVEL3,
						   ("file pat match (cascade) %s\n",
						    dent->d_name));
					...
					add_work(fullpath, FILET);
					DP(DLEVEL3,
					   ("cascade added file (MATCH) %s to Work Q\n",
					    fullpath));
				}
				break;
			case S_IFDIR:
...
void prnt_stats(void)
{
	...
	fprintf(stderr, "Number of files searched:                 %d\n",
		st_file_search);
	c = (float)(st_dir_search + st_file_search) /
	    (float)(st_end - st_start);
	...
	t += c;
	fprintf(stderr,
		"Work stopped due to TGLIMIT:  (%.9s) %d Times, %3.2f%%\n", tl,
		st_worklimit, c);
...
void notrun(void)
{
	...
	if (flags & FS_STATS) {
		...
		if (running > st_maxrun) {
			st_maxrun = running;
			DP(DLEVEL6,
			   ("Max Running has increased to %d\n", st_maxrun));
		}
		mutex_unlock(&stat_lk);
	}
	running--;
...
void *SigThread(void *arg)
{
	...
	while (1) {
		...
		default:
			DP(DLEVEL7,
			   ("Default action taken (exit) for signal %d\n",
			    sig));
...
#ifdef DEBUG
	fprintf(stderr,
		"Debug     -d = debug level -d <levels> (-d0 for usage)\n");
...
int regexp_usage(void)
{
	...
	fprintf(stderr, "\n");
	fprintf(stderr,
		"To match two diffrerent patterns in the same command\n");
	fprintf(stderr,
		"Use the or function. \n"
	...
	fprintf(stderr,
		"RegExp Pattern matching brought to you by Marc Staveley\n");
	exit(0);
}
...
void debug_usage(void)
{
	...
	fprintf(stderr, "--------------------------------------------------\n");
	fprintf(stderr,
		"You can or the levels together like -d134 for levels\n");
