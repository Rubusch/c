...
/*
  ...
  at pressing enter - after a while and numerous ENTERs the proggy runs into a
  deadlock since the mutex is still locked by the counter thread, the main
  thread put the couter thread into "suspend" and (because the mutex already is
  locked), the main thread is not able to lock the mutex for it.
//*/
...
int main()
{
	...
	if (0 !=
	    pthread_create(&tid_counter, &attr_counter, counter, (void *)0)) {
...
void *counter(void *arg)
{
	while (1) {
		...
		if (!suspend) { // this happens with a locked mutex -> dangerous, it will
			...
			++count;
			for (idx = 0; idx < 500;
			     ++idx) // the lower the number, the easier to run
