/*
  MUTEXES (used generally for synchronizing two threads, can provoke
  deadlocks with more threads)



  pthread_self() - returns the unique, system assigned thread ID of
                              the calling thread

  pthread_equal(thread_id1, thread_id2) - compares two thread IDs

  pthread_once(once_control, init_function) - executes the given
                              init_function exactly once in a process
                              once_control is a synchronization
                              control structure that requires
                              initialization prior to calling
                              pthread_once(), e.g.:

                              pthread_once_t once_control = PTHREAD_ONCE_INIT;

  pthread_yield() - forces the calling thread to relinquish use of its
                              processor and to wait in the run queue
  ...
  mutex variables must be declared with type pthrea_mutex_t and must
  be initialized befofe they can be used, mutex variables initially
  are UNLOCKED
  ...
  pthread_mutex_lock(mutex) - used by a thread to acquire a lock on
                                 the specified mutex variable this
                                 call will block other calls for the
                                 same mutex

  pthread_mutex_trylock(mutex) - if the mutex is already locked this
                                 call will return with a busy error
                                 code
...
/*
  structure contains necessary information to allow the function
  dotprod() to access its input data and place its output into the
  structure
*/
typedef struct {
	double *a;
...
/*
  the thread function, all input to this function is obtained from a
  structure of type DOTDATA and all output from this function is
  written into this structure. when a thread is created we pas a
  single argument to the activated function - typically this argument
  is a thread number
*/
...
main(int argc, char *argv[])
{
	...
	for (idx = 0; idx < NUMTHRDS; ++idx) {
		/*
		  ...
		  offset is specified by idx, the size of data for
		  each thread is indicated by VECLEN
		*/
