...
int main(int argc, char **argv)
{
	...
	unsigned char code[] = {
		0xcd, 0x80, 0xcc, 0, 0, 0, 0
	}; // NOTE: the instruction only has 3 elements, though we align it to 8 due to x86_64
	...
	for (cnt = 0; cnt < len; ++cnt)
		fprintf(stderr, "'0x%02x' ", backup[cnt]);
	fprintf(stderr, "\n");
	...
	fprintf(stderr,
		"AWESOME: we got a BREAKPOINT - the process stopped!\n");
	printf("press ENTER\n");
	getchar();
