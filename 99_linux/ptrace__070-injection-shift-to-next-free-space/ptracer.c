...
long freespaceaddr(pid_t pid)
{
	...
	while (NULL != fgets(line, 120, fp)) {
		sscanf(line, "%lx-%hhx %s", (long unsigned int *)&addr, str,
		       str);
...
int main(int argc, char *argv[])
{
	...
	fprintf(stderr,
		"XXX %s()[%d]:\t'0x%016lx' - regs.rip (original next instruction)\n",
	...
	fprintf(stderr,
		"XXX %s()[%d]:\t'0x%016lx' - addr (free address, heap)\n",
	...
	fprintf(stderr,
		"Let's shift the next instruction to addr (0x%016lx) and let it continue there..\n",
		(long)addr);
