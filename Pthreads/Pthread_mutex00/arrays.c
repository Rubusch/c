// Arrays.c
/*
  demonstrates the protection of a common data structure, used by several threads

  take a look on the order in which the working threads are handling the data structure
//*/
...
int main()
{
  ...
  for(idx = 0; idx < 5; ++idx){
    if(0 != pthread_create( (void*) &thr[idx], NULL, add_to_value, (void*) (2*idx))){
...
void *add_to_value(void *arg)
{
  ...
  /*
    ...
    observe the order of the working threads on the structure 
    in the output!
  //*/
  int idx = 0;
  for(idx = 0; idx < 5; ++idx){
    ...
    printf("%d -\tidx(%d): %d += %d ", (inval/2), (idx % 5), data.int_val[idx % 5], inval);
    data.int_val[idx % 5] += inval;
    printf("= %d\n", data.int_val[idx % 5]);

    printf("%d -\tidx(%d): %.1f += %.1f ", (inval/2), (idx % 5), data.double_val[idx % 5], ((double) 1.5 * inval));
