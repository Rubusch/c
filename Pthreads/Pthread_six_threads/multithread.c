...
/*
  ...
  Solaris: thr_join()
     ...
     2.arg - points to the id of the 1.arg thread if the join terminates successfully
     3.arg - status argument


  global pthread_t variables are named "thr_"
  local pthread_t variables are named "thread_"
  global pthread_cond_t is "cv_" and is used to demonstrate "suspend" of a thread  
//*/
...
int main()
{
  ...
  printf("MAIN: Main Created threads A:%lu B:%lu C:%lu\n", (unsigned long) thr_a, (unsigned long) thr_b, (unsigned long) thr_c); 
...
void* sub_c(void *arg)
{
  ...
  // create F
  puts("C: creates thread F now (should be as a daemon in Solaris), which is here basically a local thread ID");
  pthread_t thread_f;
