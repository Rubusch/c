...
/*
  ...
  3. set the attribute "detachestate" with pthread_attr_setdetachstate() to PTHREAD_CREATE_JOINABLE
  4. when done, free library resources used by the attribute with pthread_attr_destroy()

  
  pthread_join(thread_id, detachstate)           - joins a thread (synchronization)
  pthread_detach(thread_id, detachstate)         - detaches a thread (independance)
  pthread_attr_setdetachstate(attr, detachstate) - sets the status of a thread "detachable"/"joinable"
  pthread_attr_getdetachstate(attr, detachstate) - returns the detachstate of a thread, "detachable"/"joinable"
//*/
...
int main(int argc, char** argv)
{
  ...
  for(cnt=0; cnt<NUM_THREADS; ++cnt){
    ...
    if(0 != (return_code = pthread_create(&thread[cnt], &attr, busy_work, NULL))){
      fprintf(stderr, "ERROR; return code from pthread_create() is %d\n", return_code);
      exit(1);
    }
  }
  
  // 4. pthread_attr_destry(&attr) - free attribute and wait for the other threads, when done
