...
%macro _print 1
    ...
    pushf
    jmp %%astr
    %%str db %1,0                    ; is this actually "clean", simply to append yet another '0'?
    %%strln equ $-%%str
...
; note:
; - pusha/popa won't be available in 64 bit
;
; - issue: Relocation Truncated to Fit error in NASM x86_64
;       when using 'db' values from section .data in macros, the linker may
;       complain, a possible solution then is to move the
;       values/string into registers and pass them to the macro
;
; - issue: comma, colon, decorator or end of line expected after operand
;       make sure all , or : are placed correctly inside the macro
;
