// mq_sample.c
/*
  demonstrates msgsnd() and msgrcv() and works more or less - this is experimental grounds!!!

  <sys/msg.h> includes a definition of struct msgbuf with the mtext field defined as:
  char mtext[1]
  
  therefore, this definition is only a template, not a structure definition that you 
  can use directly, unless you want only to send and receive messages of 0 or 1 byte.
  To handle this, malloc an area big enough to obtain the template - the size of the 
  mtext template field + the size of the mtext field wanted.
  Then you can use the pointer returned by malloc as a struct msgbuf with an mtext 
  field of the size you want. Note also that sizeof(msg_ptr->mtext is valid even 
  though msg_ptr isn't pointing to anything yet. Sizeof doesn't de-reference msg_ptr, but 
...
void readlongnumber(signed long*, const unsigned int, const char*);
...
int main(int argc, char** argv)
{
  ...
  // max buffer size
  max_msg_sz = MSG_SIZE_MAX;
  /*
    readnumber(&max_msg_sz, TMP_SIZE, "enter the message buffer size you want:");
  //*/
  ...
  while(0 != (choice = ask())){
    ...
    case 1:
      /*
	msgsnd() requested: get the params, make the call and report the results
      //*/
      fprintf(stderr, "valid msgsnd message types are positive integers.\n");
	...
	do{
	  ...
	  readstring(tmp, TMP_SIZE, "enter msgflg (yes/no): ");
	  if((strlen("no") == (strlen(tmp) -1)) || (strlen("yes") == (strlen(tmp) -1))){
	    if(!strncmp(tmp, "Yes", 3) || !strncmp(tmp, "yes", 3)){
	      msg_flag = IPC_NOWAIT;
	      repeat = 0;	      
	    }else if((!strncmp(tmp, "No", 2) || !strncmp(tmp, "no", 2))){
	      msg_flag = 0;
	      repeat = 0;
	    }else{
	      repeat = 1;
      ...
      fprintf(stderr, "%s(%d, msg_ptr, %d, %#o)\n", "msgop: calling msgsnd", mq_id, msg_sz, msg_flag);
	...
	fprintf(stderr, "\"\n");
      ...
      //
      msg_sz = sizeof(*msg_ptr);
      //
      if(0 > (res = msgsnd(mq_id, msg_ptr, msg_sz, msg_flag)))
    ...
    case 2:
      ...
      msg_sz = 8; 
      /* don't ask
      readnumber(&msg_sz, TMP_SIZE, "read in the message size:")
      //*/
      
      fprintf(stderr, "msg_type meanings:\n");
      fprintf(stderr, "\t 0 %s\n", first_on_queue);
      fprintf(stderr, "\t>0 %s of given type\n", first_on_queue);
      fprintf(stderr, "\t<0 %s with type <= |msgtype|\n", first_on_queue);

      // read in message type
      readlongnumber(&msg_type, TMP_SIZE, "enter msg_type (long):");
	...
	msg_flag = atoi(tmp);
      ...
      // summary
      fprintf(stderr, "msgop: calling msgrcv(%d, msg_ptr, %d, %ld, %#o);\n", mq_id, msg_sz, msg_type, msg_flag);

      // send
      if(-1 == (res = msgrcv(mq_id, msg_ptr, msg_sz, msg_type, msg_flag))){
...
static int ask()
{
  ...
  do{
    ...
    puts("\tmsgsnd = \t1");
    puts("\tmsgrcv = \t2\n");
    readdigit(&response, "enter your choice:");
...
// READING
void readnumber(unsigned int* iTxt, const unsigned int digits, const char* comment)
{
  if(NULL == comment){
    perror("text is NULL");
    return;
...
void readlongnumber(signed long* val, const unsigned int digits, const char* comment)
{
  if(NULL == comment){
    perror("text is NULL");
    return;
