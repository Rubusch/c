...
static error_code_t
_mq_init( int *mq_id, char seed )
{
  key_t mq_key = 0;
  int mq_flags = -1;
  struct msqid_ds buf;
  
  if( !mq_id ) return ERRCODE_FAILURE;
  ...
  *mq_id = -1;
  ...
  fprintf( stderr, "\tmsg_qbytes\t= %d, Maximum number of bytes allowed in queue\n", ((int) mq_stat.msg_qbytes) );
  ...
  fprintf( stderr, "\tmsgmax\t\t= %d, Max. # of bytes that can be written in a single message\n", mq_info.msgmax );
  fprintf( stderr, "\tmsgmnb\t\t= %d, Max. # of bytes that can be written to queue; used to initialize msg_qbytes during queue creation (msgget())\n", mq_info.msgmnb );
...
static error_code_t
_mq_send( const int mq_id, struct mq_message_s *mq_message, const int mtype, const int blocking )
{ 
  if( (mtype != 2) && (mtype != 4) ){
    perror( "mtype is neither 2, nor 4, sending message to external program aborted" );
    return ERRCODE_FAILURE;
  }
...
static error_code_t
_mq_retrieve( const int mq_id, struct mq_message_s *mq_message, const int mtype, const int blocking )
{
  if( !mq_message ) return ERRCODE_FAILURE;
  mq_message->mtype = mtype;

  if( (mq_message->mtype != 1) && (mq_message->mtype != 3) ){
    perror( "mtype is neither 1, nor 3, sending message to external program aborted" );
    return ERRCODE_FAILURE;
  }
...
main( int argc, char** argv )
{
  ...
  *q_id = mq_id;
  ...
  do{
    ...
    // receive
    mq_message.mtype = 1;
    if( ERRCODE_SUCCESS != _mq_retrieve( mq_id, &mq_message, mq_message.mtype, 1 ) ){
