...
int main(int argc, char** argv)
{
  ...
  // connect
  //  register int len_adr_server = sizeof(adr_server.sun_family) + strlen(adr_server.sun_path);
  ...
  do{
    ...
    if(0 > send(sd_client, strings[idx], strlen(strings[idx]), 0)){
      ...
      // receive
      if((0 >= (bytes = recv(sd_client, strReceived, RECEIVED_SIZE, 0))) && (bytes < RECEIVED_SIZE)){
	perror("recv failed");
      }else{
