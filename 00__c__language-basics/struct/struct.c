...
{
	if (NULL == (*ppText = malloc(sizeof(**ppText))))
		return -1;
	return 0;
}

// NO NEED FOR A DOUBLED POINTER FOR ACCESSING ELEMENTS OF A STRUCT (ALLOCATION,
// FREE, ADDRESS ASSIGNMENT)!!!
int text_init(struct text_etikett *pText, char *text,
	      const unsigned int TXT_SIZE)
{
	if (NULL == pText)
		return -1;

	// init text
	if (NULL == (pText->szText = calloc(TXT_SIZE, sizeof(*pText->szText))))
		return -1;
...
int main(int argc, char **argv)
{
	...
	printf("show:\t\"%s\"\t\'%d\' - strlen: \'%li\' + 1\n", text02->szText,
	       text02->text_size, strlen(text02->szText));
