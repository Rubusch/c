...
{
	return flo->int_repr < 0;
}

int32_t raw_mantissa(float_wrap_p flo)
{
	return flo->int_repr & ((1 << (FLT_MANT_DIG-1)) - 1);
}

int32_t raw_exponent(float_wrap_p flo)
{
	return (flo->int_repr >> (FLT_MANT_DIG-1)) & 0xff;
}
...
int main()
{
...

	{
		...
		int max_ulp_diff = 0; /* for small numbers still 0, gives OK */
		if (relatively_equal(flo, 0.12345, max_ulp_diff)) {
			fprintf(stderr, "EQUAL\tflo = %1.15f equals to %1.15f (on ULP with %d)\n", flo, 0.12345, max_ulp_diff);
		} else {
			fprintf(stderr, "!!!\tflo = %1.15f is NOT equal to %1.15f (on ULP with %d)\n", flo, 0.12345, max_ulp_diff);
		}
	}

	{
		...
		flo = 228737632.0f;
		float expected;
		expected = 228737642.0;

		int max_ulp_diff = 2; /* weaker precision, still ok */

		if (relatively_equal(flo, expected, max_ulp_diff)) {
			fprintf(stderr, "EQUAL\tflo = %1.15f equals to %1.15f (on ULP with %d)\n", flo, expected, max_ulp_diff);
		} else {
			fprintf(stderr, "FAILED!!!\tflo = %1.15f is NOT equal to %1.15f (on ULP with %d)\n", flo, expected, max_ulp_diff);
		}
	}

	{
		...
		if (relatively_equal(flo, expected, max_ulp_diff)) {
			...
			fprintf(stderr, "FAILED!!! [expected] flo = %1.15f is NOT equal to %1.15f (on ULP with %d)\n", flo, expected, max_ulp_diff);
		}
	}
