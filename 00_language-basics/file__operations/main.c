...
int main(int argc, char **argv)
{
	...
	//  unsigned long int text3_size = sizeof(text3); // unused, only for
	//  debugging
	unsigned long int text4_size = sizeof(text4);
	unsigned long int content_size = LIN_LEN;

	char *content = NULL;
	if ((content = calloc(content_size, sizeof(*content))) == NULL)
		return EXIT_FAILURE;
	strcpy(content, "");
	...
	puts("11. copying buffered - without created file2 - OK (already done "
	     "before)");
	printf("Copy characterwise and buffered...\n");
	printf("%i - Copying %s to %s characterwise, buffered\n",
	       copy_characterwise_buffered(file2, file3, LIN_LEN), file2,
	       file3);
	printf("Done.\n\n");
	//*/
	...
	printf("%i - Copying %s to %s characterwise, unbuffered\n",
	       copy_characterwise_unbuffered(file2, "Jill.txt"), file2,
	       "Jill.txt");
	printf("Done.\n\n");
	//*/
  ...
  printf("%i - Check out the read/write pointer to \'%s\'\n",
  get_read_write_file_pointer(&fp, file3), file3); printf("%i - Read \'%s\'
  ...
  //*/
	...
	printf("%i - The file \'%s\' (function failed) ",
	       filesize(fp, &file_size), file3);
  ...
  printf("%i - Get a read & write pointer\n", get_read_write_file_pointer(&fp,
  file2)); printf("NO TEST HERE: THE FUNCTION IS A DEMO HOW TO WRITE STRUCTS
  DIRECTLY!\n"); printf("%i - Close stream\n", close_stream(&fp));
  printf("Done.\n\n");
  //*/
	...
	//* // reading linewise and get read file pointer - 3 lines more in file2 - OK
	puts("ASSERT: 7. reading linewise and get read file pointer - 3 lines more "
	     "in file2");
	...
	//* // reading out without spaces nor line feed - same, 3 lines more - OK
	puts("ASSERT: 10. reading out without spaces nor line feed - same, 3 lines "
	     "more");
	assert(0 == get_read_file_pointer(&fp, file2));
	...
	//* // copying buffered - without created file2 - OK (already done before)
	puts("ASSERT: 11. copying buffered - without created file2 - OK (already "
	     "done before)");
	printf("Copy characterwise and buffered...\n");
  ...
  printf("Write blockwise \'%s\'\n", file2);
  assert(0 == get_read_write_file_pointer(&fp, file2));
  printf("NO TEST HERE: THE FUNCTION IS A DEMO HOW TO WRITE STRUCTS
  DIRECTLY!\n"); assert(0 == close_stream(&fp)); printf("Done.\n\n");
  //*/
