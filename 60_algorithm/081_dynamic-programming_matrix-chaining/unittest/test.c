...
void test_memoized_matrix_chain(void)
{
	//  verifies by the book's example of six matrices
	//
	//  matrix    |   A1  |   A2  |   A3  |   A4  |   A5  |   A6
	...
	dynamic_programming_debug("\n");
	memo = memoized_matrix_chain_order(rows, cols, size);
	CU_ASSERT(NULL != memo);
...
int main(void)
{
	...
	TEST_start(pSuite, "matrix struct", test_matrix)
		...
		TEST_append(pSuite, "memoized matrix-chain order",
			    test_memoized_matrix_chain) // */
	TEST_end();
