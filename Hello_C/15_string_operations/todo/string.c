...
text:
"All work and no play makes Jack a dull boy, all play and no work make Jack a mere toy!"

pattern:
"Jack"

search for the pattern...
	0. compare: pattern "Jack" with "All" - 4 token compared
	1. compare: pattern "Jack" with "wo" - 3 token compared
	2. compare: pattern "Jack" with "k" - 2 token compared
	3. compare: pattern "Jack" with "and" - 4 token compared
	4. compare: pattern "Jack" with "no" - 3 token compared
	5. compare: pattern "Jack" with "play" - 5 token compared
	6. compare: pattern "Jack" with "makes" - 6 token compared
	7. compare: pattern "Jack" with "Jack" - 5 token compared

 -> Contains the pattern: "Jack"
...
#define TEXTSIZ 256
#define TEXT "All work and no play makes Jack a dull boy, all play and no work make Jack a mere toy!"
#define PATTERN "Jack"
...
int main()
{
  ...
  do{
    ...
    int result = 0;
    puts("\nsearch for the pattern...");
    if(1 == (result = check_text_for_pattern(text, text_size, pattern, pattern_size))){
...
//*/
int check_text_for_pattern( char* text, const unsigned long int text_size, char* pattern, const unsigned long int pattern_size)
{
  ...
  //*/
  // TODO - alloc ppWordList: one pointer
  // TODO - increment wordListSize;
  // TODO - alloc memory for the word to store in the last position in ppWordList
  // TODO - write the word into the ppWordList at the last position
  ...
  do{
    ...
    // 1. allocate space for wordListSize + 1 pointers to elements in ppWordList_temp
    // TODO
    ...
    /* 
       3. allocate memory for elements refered by each pointer already allocated in "1."
       (index: wordListSize, still not incremented!)

       only the last element will be put into new allocated space, 
    //*/
    if(NULL == (ppWordList_temp[wordListSize] = calloc((1 + strlen(word)), sizeof(*ppWordList_temp[wordListSize])))){
  ...
  if(-1 != result){
    for(cnt = 0; cnt < wordListSize; ++cnt){
      printf("\t%d. compare: pattern \"%s\" with \"%s\" - %d token compared\n", cnt, 
	     pattern, ppWordList[cnt], (1 + strlen(ppWordList[cnt]))); 
      // TODO
    }
  }
