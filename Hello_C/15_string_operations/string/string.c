...
int check_text_for_pattern( char*, const unsigned long int, char*, const unsigned long int);
...
int check_text_for_pattern( char* text, const unsigned long int text_size, char* pattern, const unsigned long int pattern_size)
{
  ...
  ++wordListSize;
  if(NULL == (ppWordList[wordListSize - 1] = calloc((1+strlen(word)), sizeof(*ppWordList[wordListSize - 1])))){
  ...
  do{
    ...
    if(NULL == (ppWordList_temp = calloc((wordListSize + 1), sizeof(*ppWordList_temp)))){
  ...
  if(-1 != result){
    for(cnt = 0; cnt < wordListSize; ++cnt){
      printf("\t%d. compare: pattern \"%s\" with \"%s\" - %d token compared\n", cnt, pattern, ppWordList[cnt], (1 + strlen(ppWordList[cnt]))); // XXX
      if(0 == strncmp(pattern, ppWordList[cnt], (1 + strlen(ppWordList[cnt])))){		
	result = 1;
	break;
      }
    }
