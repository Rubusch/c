/*
  ...
  This is kind of the most common structure, a pointer to arrays or an array of a pointer.
  here is just one as example: a pointer to arrays.

  Conclusion:

  char** ptr; // has a first pointer (unsigned long int because of the address size!) and a 
  second pointer to a value of the type itself, here a 'char'.
  ...
  *ptr;   // the content of the first pointer, the address of the char value (unsigned long int)
...
/***
    Can I use a void ** pointer to pass a generic pointer-to-pointer to a set of functions 
    by reference?
***/

/*
  Not portably. There is no generic pointer-to-pointer type in C. void* acts as a generic 
  pointer only because conversions are applied automatically when other pointer types are 
  assigned to and from void*'s; these conversions cannot be performed (the correct underlying 
  pointer type is unknown!!!) if an attempt is made to indirect upon a void** value which 
  points at something other than a void*.
//*/
