...
//*/
int backend_insert_node(leaf* item)
{
  return insert(backend_get_root(), item);
}
...
/*
  ...
  asks and uses node ptr directly do pass the pointer's address to the delete function!
//*/
...
int backend_find_and_delete_node(leaf* item)
{
  ...
  if(delnode->data == item->data){
    if((NULL == backend_get_left_node(delnode)) && (NULL == backend_get_right_node(delnode))){
      backend_delete_node(&item);
      return 0;
    }else{
      return -1;
    }
  }else if(NULL != (delnode = find((backend_get_root())->left, &prevnode, item->data))){
    // ok found, next step: delete
  }else if(NULL != (delnode = find((backend_get_root())->right, &prevnode, item->data))){
    // ok found, next step: delete
  }else{
    return -1;
  }
...
//*/
unsigned int backend_get_data(leaf* item)
{
  return item->data;
}
...
{
  if(NULL == (node_ptr = item)) return 0;
  else return -1;
}
