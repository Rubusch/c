...
{
  if(NULL != backend_get_root()){
    fprintf(stderr, "ERROR: init_root() - backend_get_root() is NOT NULL\n");
    return -1;
  }
  if(NULL == elements){
    fprintf(stderr, "ERROR: init_root() - number of elements is NULL\n");
    return -1;
  }
...
{
  if(NULL == backend_get_root()){
    fprintf(stderr, "ERROR: populate_tree() - backend_get_root() is NULL\n");
    return -1;
  }
  if(NULL == elements){
    fprintf(stderr, "ERROR: populate_tree() - number of elements is NULL\n");
    return -1;
  }
  ...
  for(cnt=0; (number > cnt) && (MAX_NODES > *elements); ++idx_break){
    if(0 == backend_insert_node( backend_set_data_automatically(backend_create_node()))){ 
      ++(*elements);
      ++cnt;
    }

    if(MAX_NODES <= idx_break) break;
  }
...
{
  if(NULL == backend_get_root()){
    fprintf(stderr, "ERROR: add_element() - backend_get_root() is NULL\n");
    return -1;
  }
  if(NULL == number){
    fprintf(stderr, "ERROR: add_element() - number of elements is NULL\n");
    return -1;
  }
  if(MAX_NODES <= *number){
    fprintf(stderr, "ERROR: add_element() - maximum number of nodes, not inserting\n");
    return -1;
  }
...
{  
  if(NULL == backend_get_root()){
    fprintf(stderr, "ERROR: find_element() - backend_get_root() is NULL\n");
    return -1;
  }

  backend_set_node_ptr(backend_create_node());
  if(NULL != backend_find_node(backend_set_data(backend_get_node_ptr()))){
...
{
  if(NULL == backend_get_root()){
    fprintf(stderr, "ERROR: delete_leaf() - backend_get_root() is NULL\n");
    return -1;
  }
  if(NULL == number){
    fprintf(stderr, "ERROR: delete_leaf() - number of elements is NULL\n");
    return -1;
  }

  backend_set_node_ptr(backend_create_node());
  if(0 > backend_find_and_delete_node(backend_set_data(backend_get_node_ptr()))){
    backend_free_node_ptr();
    return -1;
  }
  backend_free_node_ptr();
...
{
  if(NULL == backend_get_root()){
    fprintf(stderr, "ERROR: delete_tree() - backend_get_root() is NULL\n");
    return -1;
  }
     
  if(NULL == number){
    fprintf(stderr, "ERROR: delete_tree() - number of elements is NULL\n");
    return -1;
  }
...
//*/
int init_path(char* path, const unsigned int idx_row, const unsigned int path_size)
{
...
/*
  ...
  or -2
//*/
int get_data_by_path(char* data, const unsigned int data_len
  ...
  if(idx_col % 2 != 0) return 0;
  backend_set_node_ptr(backend_get_root());

  // get element
  unsigned int idx_path=0;
  for(idx_path=0; (idx_path <= idx_col/2) && (NULL != backend_get_node_ptr()); ++idx_path){
    if(path[idx_path] == '1'){
      backend_set_node_ptr(backend_get_right_node(backend_get_node_ptr()));
    }else if(path[idx_path] == '0'){
      backend_set_node_ptr(backend_get_left_node(backend_get_node_ptr()));
    }else{
      break;
    }
  } 
  ...
  if(NULL == backend_get_node_ptr()){
    ...
    // convert to string
    data_to_string(data, (data_len+1), backend_get_data(backend_get_node_ptr()));
  }
...
int write_tree()
{
  if(NULL == backend_get_root()) return -1;
  ...
  const unsigned int line_size = (idx_col_max / 2 + 1) * data_len + (idx_col_max / 2 + 1) + 2;
  ...
  for(idx_row = 0; idx_row <= idx_row_max; ++idx_row){
    ...
    // set up mask
    if(0 > get_paint_mask_per_row(paint, idx_col_max+1, idx_row_max, idx_row)){
	...
	if(0 < paint[idx_col]){
	  // row to write data
	  get_data_by_path(data, data_len, path, idx_col_max/2 + 1, idx_col); 
	}else{
	  // no data to write
	...
	if(0 < paint[idx_col]){ 
	  ...
	  else if((idx_col_max > 3) && (idx_col == idx_col_max - 3)){
	    if((idx_row > 1) && (((idx_row - 2) % 8) == 0)){
	      data[0] = '/';
	    }else{
	      data[0] = '\\';
	    }
...
{
  if(NULL == number_of_elements) return -1;

  backend_set_node_ptr(backend_get_leftest());
  unsigned int cnt=0;
  for(cnt=0; cnt < *number_of_elements; ++cnt){
    backend_set_node_ptr(backend_traverse_pre_order(backend_get_node_ptr())); 
...
int traverse_post_order(unsigned int* number_of_elements)
{
  ...
  unsigned int cnt=0;
  for(cnt=0; cnt < *number_of_elements; ++cnt){
    backend_set_node_ptr(backend_traverse_post_order(backend_get_node_ptr())); 
    if(0 > backend_print_data(backend_get_node_ptr())){
      return -1;
    }
  }
...
int traverse_in_order(unsigned int* number_of_elements)
{
  ...
  unsigned int cnt=0;
  for(cnt=0; cnt < *number_of_elements; ++cnt){
    backend_set_node_ptr(backend_traverse_in_order(backend_get_node_ptr())); 
    if(0 > backend_print_data(backend_get_node_ptr())){
      return -1;
    }
  }
...
{
  backend_set_node_ptr(backend_create_node());
  if(0 > backend_left_rotation(backend_set_data(backend_get_node_ptr()))){
    backend_free_node_ptr();
    return -1;
  }
  backend_free_node_ptr();
...
{
  backend_set_node_ptr(backend_create_node());
  if(0 > backend_right_rotation(backend_set_data(backend_get_node_ptr()))){
    backend_free_node_ptr();
    return -1;
  }
  backend_free_node_ptr();
