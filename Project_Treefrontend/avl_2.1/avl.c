...
void *
avl_find (const struct avl_tree *tree, const void *item, int flag)
{
	...
	if (diff < 0)
	{
		...
		{
		    diff =
			tree->avl_compare (p->avl_data, ref->avl_data,
					   tree->avl_param);
		    if (diff < 0)
			ref = p;
		}
...
/* KGB */
void *
avl_find_and_traverser_init (const struct avl_tree *tree, const void *item,
...
void **
avl_probe (struct avl_tree *tree, void *item)
{
...
void *
avl_insert (struct avl_tree *tree, void *item)
{
...
void *
avl_replace (struct avl_tree *tree, void *item)
{
    void **p = avl_probe (tree, item);
    if (*p == item)
	return NULL;
...
void *
avl_delete (struct avl_tree *tree, const void *item)
{
    /* Stack of nodes. */
...
void *
avl_first (struct avl_tree *tree, struct avl_traverser *trav)
{
...
void *
avl_next (struct avl_traverser *trav)
{
...
/* Copies |org| to a newly created tree, which is returned.
   ...
   and returns |NULL|. */
struct avl_tree *
avl_copy (const struct avl_tree *org, avl_copy_func * copy,
...
/* Frees storage allocated for |tree|.
   ...
   order. */
void
avl_destroy (struct avl_tree *tree, avl_node_func * destroy)
{
