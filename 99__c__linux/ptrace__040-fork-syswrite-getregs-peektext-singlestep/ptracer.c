...
int main(int argc, char **argv)
{
	...
	if (child == 0) {
		...
		while (1) {
			...
			if (start == 1) {
				// get ins by regs.eip
				ins = ptrace(PTRACE_PEEKTEXT, child, regs.rip,
					     NULL);

				// read out string argument from program
				str = (char *)calloc(
					(regs.rdx + 1),
					sizeof(char)); // size of string is in rdx
				memset(str, '\0', (regs.rdx + 1));
				int idx = 0;
				for (idx = 0; idx < (regs.rdx + 1); ++idx) {
					str[idx] = ptrace(
						PTRACE_PEEKTEXT, child,
						regs.rsi + idx,
						NULL); // start address of string is in rsi
				}
				str[regs.rdx - 1] =
					'\0'; // display cosmetics: replace \n by \0

				// display
				fprintf(stderr,
					"RIP: 0x%lx Instruction executed: 0x%04lx [ins], '%s' [string], 0x%04lx [rsi, start address of string], %lld [rdx, size of string]\n",
					(long)regs.rip, (long)ins, str,
					(long)regs.rsi, regs.rdx);
			...
			if (start == 1) {
				// get ins by regs.eip
				ins = ptrace(PTRACE_PEEKTEXT, child, regs.eip,
					     NULL);
