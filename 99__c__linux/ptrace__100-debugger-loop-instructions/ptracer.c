...
int main(int argc, char *argv[])
{
	...
	if ((tracee_pid != (res = waitpid(tracee_pid, &stat, WUNTRACED))) ||
	    !(WIFSTOPPED(stat))) {
	...
	while (1) {
		...
		if (0 >
		    (pres = ptrace(PTRACE_SINGLESTEP, tracee_pid, 0, signo))) {
		...
		if ((signo = WSTOPSIG(stat)) == SIGTRAP) {
			fprintf(stderr,
				"DEBUG %s()[%d]:\t0x%016x - signo (SIGTRAP)\n",
		...
		if ((signo == SIGHUP) || (signo == SIGINT)) {
			...
			fprintf(stderr,
				"DEBUG %s()[%d]:\t0x%016x - signo (SIGHUP or SIGINT). Child took a SIGHUP or SIGINT. We are done\n",
				__func__, __LINE__, signo);
			break;
		}
...
// */
		...
		fprintf(stderr,
			"Stat 0x%x IP 0x%x [0x%016lx] SP x0%x [0x%016lx]  Last syscall: '%s'\n",
			stat, ip,
			ptrace(PTRACE_PEEKUSER, tracee_pid, 8 * RIP, 0), sp,
			ptrace(PTRACE_PEEKUSER, tracee_pid, 8 * RSP, 0),
			syscall == SYS_restart_syscall ?
				"SYS_restart_syscall" :
				(syscall == 0xffffffffffffffff ?
					 "unknown" :
					 "SYS_something..."));
