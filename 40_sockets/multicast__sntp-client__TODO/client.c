...
#define _XOPEN_SOURCE                                                          \
	600 /* struct iconf, freeaddrinfo(), getaddrinfo(), gai_strerror() */
...
void sntp_proc(char *buf, ssize_t num, struct timeval *nowptr)
{
	...
	fprintf(stdout, "\nv%d, mode %d, strat %d, ", version, mode,
		ntp->stratum);
	if (mode == MODE_CLIENT) {
...
int main(int argc, char **argv)
{
	...
	fd_sock = lothars__udp_client("10.0.2.2", "ntp", (void *)&mcastsa,
				      &salen);
	...
	for (ifi = _get_ifi_info(mcastsa->sa_family, 1); ifi != NULL;
	     ifi = ifi->ifi_next) {
		if (ifi->ifi_flags & IFF_MULTICAST) {
			lothars__mcast_join(fd_sock, mcastsa, salen,
					    ifi->ifi_name, 0);
			fprintf(stdout, "joined %s on %s\n",
				lothars__sock_ntop(mcastsa, salen),
				ifi->ifi_name);
		}
	}
#endif
	...
	while (1) {
		len = salen;
		num = lothars__recvfrom(fd_sock, buf, sizeof(buf), 0, from,
					&len);
