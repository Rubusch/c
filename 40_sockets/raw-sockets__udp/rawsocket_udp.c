/*
  Demo: UDP on raw socket

  The demo shows sending a reimplemented UDP datagram packet through a
...
{
	int fd_sock = 0;
	char packet_buffer[PACKET_BUFFER_SIZE]; // no data/payload just datagram
	struct ipheader *ip = NULL; // re-implemented header structures
	...
	ip->iph_ver = 4;
	ip->iph_tos = 16; // low latency
	ip->iph_len = sizeof(*ip) + sizeof(*udp);
