...
{
  if( (*ppText = malloc(sizeof(**ppText))) == NULL) return -1;
  return 0;
}
...
int text_init(struct text_etikett* pText, char* text, const unsigned int TXT_SIZE)
{
  ...
  // init text
  if((pText->szText = calloc(TXT_SIZE, sizeof(*pText->szText))) == NULL) return -1;
