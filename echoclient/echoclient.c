...
main( int argc, char** argv )
{
  ...
  int nbytes = 0;
  int read = 0;
  char server_ip[16]; memset( server_ip, '\0', 16 );
  char server_port[6]; memset( server_port, '\0', 6 );
  char text[4096]; memset( text, '\0', 4096 );

  strcpy( server_ip, "192.168.60.7" );
  strcpy( server_port, "12345" );

  strcpy( text, "<?xml version=\"1.0\"?> \n<TASK task_name=\"guiagent\">\n  <DATA_OUT name=\"xxx\" dim=\"0\" type=\"cmd\" value=\"agtstart\">\n <DST name=\"agentname\" />\n </DATA_OUT>\n </TASK>" );

  puts("socket()");
  if( 0 > (sockfd = socket( PF_INET, SOCK_STREAM, IPPROTO_TCP ) ) ){ // PF_INET for packets
