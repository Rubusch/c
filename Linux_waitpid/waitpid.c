...
/*
  ...
  As wait(), but wait's on a particular child process (pid) which 
  doesn't need to be the first one.

params:
  pid - pid of the child process
  status - ???
  options - 3 possibilities:
      WCONTINUED func shall report the status of any continued child 
                 process specified by pid whose status has not been 
		 reported since it continued from a job control stop

      WNOHANG    func shall not suspend execution of the calling thread 
                 of status is not immediately available for the one of 
		 the child process pid

      WUNTRACED  status of any child processes specified by pid that 
                 are stopped shall be reported to the requested process


Macros:
  WIFEXITED()    evaluates to a non-zero value if "status" was returned 
                 for a child process that terminated normally

  WEXITSTATUS()  if the value is non-zero, this macro evaluates to the 
                 low-order 8 bits of "status" that the child process passed 
		 to _exit() or exit(), or the value the child pid returned 
		 from main()

  WIFSIGNALED()  evaluates to a non-zero value of "status"was returned for 
                 a child process that terminated due to the receipt of a 
		 signal that was not caught

  WTERMSIG()     if WIFSIGNALED is non-zero, this macro evaluates to the 
                 number of the signal that caused the termination of the 
		 child pid
...
int main()
{
  ...
  if(0 > (pid = fork())){
    ...
    if( WIFEXITED(childExitStatus)){
      printf("%schild exited with exit(): %i\r\n", identifier, WEXITSTATUS(childExitStatus));
