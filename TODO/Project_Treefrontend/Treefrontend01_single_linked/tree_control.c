...
int populate_tree(unsigned int *elements, const unsigned int number)
{
  ...
  for (cnt = 0; (number > cnt) && (MAX_NODES > *elements); ++idx_break) {
    if (0 == backend_insert_node(
                 backend_set_data_automatically(backend_create_node()))) {
      ++(*elements);
      ++cnt;
    }
...
int delete_leaf(unsigned int *number)
{
  ...
  backend_set_node_ptr(backend_create_node());
  if (0 >
      backend_find_and_delete_node(backend_set_data(backend_get_node_ptr()))) {
    backend_free_node_ptr();
    return -1;
  }
  backend_free_node_ptr();
  ...
  for (idx_col = 0; idx_col < paint_size; ++idx_col) {
    if (0 == idx_col % 2) {
      ...
      if ((idx_row >= (shift_data / 2 - 1)) &&
          (0 == (idx_row + 1 - (shift_data / 2)) % shift_data)) {
        paint[idx_col] = 1;
      } else {
        paint[idx_col] = 0;
      }
      ...
      if (idx_col == paint_size - 2) {
        ...
        if ((idx_row + 1 != interval) &&
            ((idx_row_shifted % (2 * interval)) < (interval - 1)) &&
            (paint[idx_col - 1] != 1)) {
          paint[idx_col] = 1;
        } else {
          paint[idx_col] = 0;
...
int write_tree()
{
  ...
  for (idx_row = 0; idx_row <= idx_row_max; ++idx_row) {
    ...
    // set up mask
    if (0 >
        get_paint_mask_per_row(paint, idx_col_max + 1, idx_row_max, idx_row)) {
