...
void *avl_find(const struct avl_tree *tree, const void *item, int flag)
{
	...
	for (p = tree->avl_root; p != NULL;) {
		int diff =
			tree->avl_compare(item, p->avl_data, tree->avl_param);
		...
		if (diff < 0) {
			if (flag == AVL_GE) {
				if (ref == NULL)
					...
					diff = tree->avl_compare(
						p->avl_data, ref->avl_data,
						tree->avl_param);
					if (diff < 0)
						ref = p;
				}
				  ...
				  int flag, struct avl_traverser *trav)
{
	...
	for (p = tree->avl_root; p != NULL;) {
		int diff = tree->avl_compare(avl_data, p->avl_data,
					     tree->avl_param);

		if (diff < 0) {
			if (trav)
				trav->avl_stack[trav->avl_height++] =
					(struct avl_node *)p;
			p = p->avl_link[0];
		} else if (diff > 0) {
			p = p->avl_link[1];
	...
	for (;;) {
		...
		for (;;) {
			...
			if (x->avl_link[1] != NULL) {
				y->avl_link[1] =
					avl_allocate(sizeof *y->avl_link[1]);
