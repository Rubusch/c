...
int main(int argc, char **argv)
{
  ...
  case 6:
    ...
    readnumber(( unsigned int * )&sem_id_ds.sem_perm.mode, SEM_PERM_MODE_DIGITS,
  ...
  case 8:
    ...
    if (NULL ==
        (sem_un.array = calloc(sem_id_ds.sem_nsems, sizeof(*(sem_un.array))))) {
      perror(
          "calloc failed - unable to allocate space for sem_id_ds.sem_nsems");
      exit(EXIT_FAILURE);
    }
    // xxx
    {
      ...
      for (idx = 0; idx < sem_id_ds.sem_nsems; ++idx) {
        ...
        readnumber(( unsigned int * )&sem_un.array[idx], SEM_UN_ARRAY_DIGITS,
                   "value:");
      }
    }
    do_semctl(sem_id, 0, sem_cmd, sem_un);
...
static void do_stat()
{
  ...
  fprintf(stderr, "sem_otime = %s",
          sem_id_ds.sem_otime ? ctime(&sem_id_ds.sem_otime) : "not set\n");
