  ...
  do {
    ...
    for (idx = 0; idx < num_sem_ops; ++idx) {
      ...
      fprintf(stderr, "sem_num(valid vlaues are 0 <= sem_num < %ld):\n",
              sem_id_ds.sem_nsems);
      readnumber(( unsigned int * )&sem_ops[idx].sem_num, SEM_NUM_DIGITS,
                 "enter a sem_num value:");

      fprintf(stderr, "sem_op\n");
      readnumber(( unsigned int * )&sem_ops[idx].sem_op, SEM_OP_DIGITS,
                 "enter a sem_op value:");
  ...
  if (sem_id < 0) {
    ...
    *pSem_id = sem_id;
    ...
    if (0 > semctl(sem_id, 0, IPC_STAT, sem_un)) {
      ...
      if (NULL ==
          (sem_un.array = calloc(sem_id_ds.sem_nsems, sizeof(*sem_un.array)))) {
