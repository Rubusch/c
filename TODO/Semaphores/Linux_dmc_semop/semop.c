	...
	do {
		...
		for (idx = 0; idx < num_sem_ops; ++idx) {
			fprintf(stderr,
				...
				idx + 1, num_sem_ops);
			fprintf(stderr,
				"sem_num(valid vlaues are 0 <= sem_num < %ld):\n",
				sem_id_ds.sem_nsems);
			readnumber((unsigned int *)&sem_ops[idx].sem_num,
				   SEM_NUM_DIGITS, "enter a sem_num value:");

			fprintf(stderr, "sem_op\n");
			readnumber((unsigned int *)&sem_ops[idx].sem_op,
				   SEM_OP_DIGITS, "enter a sem_op value:");
		...
		fprintf(stderr, "calling semop(%d, &sem_ops, %d)\n", sem_id,
			num_sem_ops);
		for (idx = 0; idx < num_sem_ops; ++idx) {
			fprintf(stderr, "sem_ops[%d].sem_num = %d, ", idx,
				sem_ops[idx].sem_num);
...
static int ask(unsigned int *pSem_id, struct sembuf **pSem_ops)
{
	...
	if (sem_id < 0) {
		// first call - get sem_id from user and the current state of the semaphore
		// set
		...
		*pSem_id = sem_id;
		...
		if (0 > semctl(sem_id, 0, IPC_STAT, sem_un)) {
			...
			if (NULL ==
			    (sem_un.array = calloc(sem_id_ds.sem_nsems,
						   sizeof(*sem_un.array)))) {
	...
	// find out how many operations are going to be done in the next call and
	// allocate enough space to do it
	unsigned int tmp_num_sem_ops = 0;
	readdigit(&tmp_num_sem_ops,
		  "enter the number of semaphores (one digit, 0 to exit)");
	if (tmp_num_sem_ops > num_sem_ops) {
		if (num_sem_ops && (sem_ops != NULL))
			free(sem_ops);
