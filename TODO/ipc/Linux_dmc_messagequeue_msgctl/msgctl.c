...
int main(int argc, char** argv)
{
  ...
  do{
    ...
    case 1:
      puts("IPC_SET");
      // IPC_SET - first get the content of the msqid_ds struct and show its content
      mq_cmd = IPC_SET;
    ...
    case 2:
      puts("IPC_STAT");
      // get a copy of the current message queue control struture and show it to the user
      ...
      //	fprintf(stderr, "msqid_ds.msg_cbytes = %d\n", buf.msg_cbytes); // check NA?!
      fprintf(stderr, "msqid_ds.msg_qbytes = %d (max number of bytes on queue)\n", (unsigned short) buf.msg_qbytes);
      ...
      if(mq_cmd != IPC_SET){
	// show the IPC_STAT content in case IPC_STAT was chosen originally - break here
	break;
      }
      ...
      readnumber((unsigned int*) &buf.msg_perm.mode, TMP_SIZ, "enter msg_perm.mode:"); // FIXME
      readnumber((unsigned int*) &buf.msg_qbytes, TMP_SIZ, "enter msg_qbytes:"); // FIXME
...
static void do_msgctl(int mq_id, int cmd, struct msqid_ds* buf)
{
  ...
  fprintf(stderr, "calling msgctl(%d, %d, %s)\n", mq_id, cmd, buf ? "&buf":"(struct msqid_ds*) NULL");
