...
int main(int argc, char **argv)
{
	...
	if ((text[1] = malloc(sizeof(char) *
			      (strlen("Jack fell down and broke his crown "
	...
	if ((text[3] = malloc(sizeof(char) *
			      (strlen("He went to bed and bound his head "
	...
	for (cnt = 0; cnt < TEXT_SIZE; ++cnt) {
		printf("%d.line - CHECKING: \n\"%s\"\n", cnt,
		       text[cnt]); // DEBUG
		if (check_line_for_pattern(text[cnt], strlen(text[cnt]),
			   ...
			   char *pattern, const unsigned long int PATTERN_SIZE)
{
	...
	do {
		...
		if ((ppWordListTemp = malloc((wordListSize + 1) *
					     sizeof(char *))) == NULL)
			return 0;
