...
int main(int argc, char **argv)
{
  ...
  printf("Strtok - separate a string into tokens\n\ninit a char** with some "
         "text:\n");
  char **text;
  if ((text = malloc(sizeof(char *) * 4)) == NULL)
    return EXIT_FAILURE;
  if ((text[0] = malloc(
           sizeof(char) *
           (strlen("Jack and Jill went up the hill to fetch a pail of water") +
            1))) == NULL)
    return -1;
  strcpy(text[0], "Jack and Jill went up the hill to fetch a pail of water");
  if ((text[1] =
           malloc(sizeof(char) * (strlen("Jack fell down and broke his crown "
  ...
  if ((text[2] = malloc(
           sizeof(char) *
           (strlen("Up got Jack, and home did trot as fast as he could caper") +
            1))) == NULL)
    return -1;
  strcpy(text[2], "Up got Jack, and home did trot as fast as he could caper");
  if ((text[3] =
           malloc(sizeof(char) * (strlen("He went to bed and bound his head "
  ...
  for (cnt = 0; cnt < TEXT_SIZE; ++cnt) {
    ...
    if (check_line_for_pattern(text[cnt], strlen(text[cnt]), pattern,
                               strlen(pattern))) {
                           ...
                           char *pattern, const unsigned long int PATTERN_SIZE)
{
  ...
  if ((word = strtok(temp, &token)) == NULL)
    return 0;

  // allocate space for one new pointer in the list, allocate space by the size
  // of word for this pointer, append to the list, and increment counter
  if ((ppWordList = ( char ** )malloc(sizeof(char *))) == NULL)
    return 0;
  // TODO: better sizeof(*ppWordList) ?
  ++wordListSize;

  if ((ppWordList[wordListSize - 1] =
           malloc(sizeof(char) * (strlen(word) + 1))) == NULL)
    return 0;
  strcpy(ppWordList[wordListSize - 1],
         word); // GDB - check ppWordList, wordListSize, word
  do {
    // reset the temp list pointer
    ppWordListTemp = NULL;

    // use strtok() with NULL, because already inited
    if ((word = strtok(NULL, &token)) == NULL)
      break;

    // reallocate space for one new pointer in the list
    // therefore allocate a temp pointer, assign the addresses of the list to
    // this and assign the ppWordListTemp's address to the ppWordList pointer
    if ((ppWordListTemp = malloc((wordListSize + 1) * sizeof(char *))) == NULL)
      return 0;

    unsigned int cnt;
    for (cnt = 0; cnt < wordListSize; ++cnt)
      ppWordListTemp[cnt] = ppWordList[cnt];
    // ppWordListTemp points to the same address!!

    // allocate ppWordListTemp[wordListSize] for one new element (index:
    // wordListSize, still not incremented!) the last element will be put into
    // new allocated space
    if ((ppWordListTemp[wordListSize] =
             malloc(strlen(word) * (sizeof(char) + 1))) == NULL)
      ;
