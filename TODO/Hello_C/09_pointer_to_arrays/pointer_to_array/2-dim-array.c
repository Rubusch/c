...
a generic pointer only because conversions are applied automatically when other
pointer types are assigned to and from void*'s; these conversions cannot be
performed (the correct underlying pointer type is unknown!!!) if an attempt is
made to indirect upon a void** value which points at something other than a
void*.
//*/
