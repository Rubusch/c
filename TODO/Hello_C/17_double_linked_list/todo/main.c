...
int main()
{
	...
	do {
		...
		readnumber(
			&strsize, 2,
			"Enter the maximum size of the strings stored as \"data\" in "
			"the list.");
    ...
    //*/
		...
		if (iSelect == 1) {
			puts("create list (1)");
			readstring(ptr_data, strsize,
				   "Enter some text as data content.");
			...
			readstring(ptr_data, strsize,
				   "Enter some text as data content.");
			readstring(ptr_prev_data, strsize,
				   "After which element to place?");
			if (0 == insertelement(ptr_prev_data, strsize, ptr_data,
					       strsize)) {
				printlist();
				puts("Ok.");
			} else {
