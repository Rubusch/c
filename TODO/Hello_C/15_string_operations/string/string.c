...
int main()
{
  ...
  //*/
    ...
    //*/
		...
		// allocation
		if (NULL == (pattern = calloc((1 + strlen(PATTERN)),
					      sizeof(*pattern)))) {
			   ...
			   char *pattern, const unsigned long int pattern_size)
{
  ...
  //*/
	...
	++wordListSize;
	if (NULL == (ppWordList[wordListSize - 1] =
			     calloc((1 + strlen(word)),
				    sizeof(*ppWordList[wordListSize - 1])))) {
  ...
  //*/
    ...
    //*/
		...
		// ppWordList_temp
		if (NULL ==
		    (ppWordList_temp = calloc((wordListSize + 1),
					      sizeof(*ppWordList_temp)))) {
	...
	if (-1 != result) {
		for (cnt = 0; cnt < wordListSize; ++cnt) {
			...
			if (0 == strncmp(pattern, ppWordList[cnt],
