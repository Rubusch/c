...
text:
"All work and no play makes Jack a dull boy, all play and no work make Jack a
mere toy!"
...
int main()
{
  ...
  //*/
    ...
    //*/
		...
		int result = 0;
		puts("\nsearch for the pattern...");
		if (1 == (result = check_text_for_pattern(
				  text, text_size, pattern, pattern_size))) {
			   ...
			   char *pattern, const unsigned long int pattern_size)
{
  ...
  //*/
	...
	// TODO - alloc memory for the word to store in the last position in
	// ppWordList
  ...
  //*/
    ...
    //*/
		if (NULL == (ppWordList_temp[wordListSize] = calloc(
				     (1 + strlen(word)),
