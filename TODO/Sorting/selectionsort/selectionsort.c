// selectionsort.c
/*
  Selection sort is in-place (doesn't need further ressources proportional to
the amount of elements to sort).

  Selection sort is NOT stable (an alphabetic order of elements, equal by
another key, e.g. birth dates sorted another time by that second key, doesn't
need to be the same alphabetic order). It can be implemented stable, too.
//*/
...
/*
  find the minimum elment, starting from "first" and using the compare function,
...
/*
  ...
  the elements will be switched in their order and the pointers will be reset so
that an impression will be invoked that only the contents of the elements have
changed.
//*/
