    ...
    case 8:                                                                    \
      ...
      *(( unsigned long long * )SUB(base_pointer, jdx)) = ull;                 \
      break;                                                                   \
                                                                               \
    default:                                                                   \
      for (number_of_elements = 0; number_of_elements < width;                 \
           ++number_of_elements) {                                             \
        /* set using offset */                                                 \
        uc = (( unsigned char * )SUB(base_pointer, idx))[number_of_elements];  \
        /* swap */                                                             \
        (( unsigned char * )SUB(base_pointer, idx))[number_of_elements] =      \
            (( unsigned char * )SUB(base_pointer, jdx))[number_of_elements];   \
...
static void *_quicksort(void *arg)
{
  ...
  case 2:
    if ((*compar_func)(SUB(base_pointer, idx), SUB(base_pointer, idx + 1)) >
        0) {
  ...
  case 3:
    ...
    // the first two are now ordered, now order the second two
    if ((*compar_func)(SUB(base_pointer, idx + 2), SUB(base_pointer, idx + 1)) <
        0) {
      SWAP(base_pointer, idx + 2, idx + 1, width);
    }

    // should the second be moved to the first?
    if ((*compar_func)(SUB(base_pointer, idx + 1), SUB(base_pointer, idx)) <
        0) {
               ...
               int (*compar_func)(const void *, const void *))
{
  ...
  if (number_of_processors == -1) {
    ...
    threads_avail =
        (number_of_processors == 1) ? 0 : (number_of_processors * THR_PER_LWP);
  }
