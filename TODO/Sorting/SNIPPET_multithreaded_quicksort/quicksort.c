...
#define SWAP(base_pointer, idx, jdx, width)                                    \
	{                                                                      \
		...
		case 1:                                                        \
			...
			*((unsigned char *)SUB(base_pointer, idx)) =           \
				*((unsigned char *)SUB(base_pointer, jdx));    \
		...
		case 4:                                                        \
			...
			*((unsigned long *)SUB(base_pointer, idx)) =           \
				*((unsigned long *)SUB(base_pointer, jdx));    \
			*((unsigned long *)SUB(base_pointer, jdx)) = ul;       \
			break;                                                 \
                                                                               \
		case 8:                                                        \
			ull = *((unsigned long long *)SUB(base_pointer, idx)); \
			*((unsigned long long *)SUB(base_pointer, idx)) = *(   \
				(unsigned long long *)SUB(base_pointer, jdx)); \
			*((unsigned long long *)SUB(base_pointer, jdx)) = ull; \
			break;                                                 \
                                                                               \
		default:                                                       \
			for (number_of_elements = 0;                           \
			     number_of_elements < width;                       \
			     ++number_of_elements) {                           \
				/* set using offset */                         \
				uc = ((unsigned char *)SUB(                    \
					base_pointer,                          \
				...
				((unsigned char *)SUB(                         \
					...
					((unsigned char *)SUB(                 \
						base_pointer,                  \
						jdx))[number_of_elements];     \
				/* reset by using offset */                    \
				((unsigned char *)SUB(                         \
					base_pointer,                          \
					jdx))[number_of_elements] = uc;        \
			}                                                      \
			break;                                                 \
		}                                                              \
	}
...
static void *_quicksort(void *arg)
{
	...
	case 2:
		if ((*compar_func)(SUB(base_pointer, 0), SUB(base_pointer, 1)) >
		    0) {
	...
	case 3:
		...
		if ((*compar_func)(SUB(base_pointer, 2), SUB(base_pointer, 1)) <
		    0) {
			SWAP(base_pointer, 2, 1, width);
		}

		// should the second be moved to the first?
		if ((*compar_func)(SUB(base_pointer, 1), SUB(base_pointer, 0)) <
		    0) {
	...
	default:
		if (number_of_elements > 3) {
			...
			if ((*compar_func)(tmp_base[0], tmp_base[2]) != 0) {
				if ((*compar_func)(tmp_base[0], tmp_base[1]) <
				    0) {
					pivot = tmp_base[1];
				} else {
					pivot = tmp_base[2];
	...
	if (pivot == 0) {
		for (idx = 1; idx < number_of_elements; ++idx) {
			if (0 != (result = ((*compar_func)(SUB(base_pointer, 0),
							   SUB(base_pointer,
							       idx))))) {
				pivot = (result > 0) ? SUB(base_pointer, 0) :
						       SUB(base_pointer, idx);
				break;
			}
		}
	}
  ...
  //*/
	...
	case 2:
		if ((*compar_func)(SUB(base_pointer, idx),
				   SUB(base_pointer, idx + 1)) > 0) {
	...
	case 3:
		...
		// the first two are now ordered, now order the second two
		if ((*compar_func)(SUB(base_pointer, idx + 2),
				   SUB(base_pointer, idx + 1)) < 0) {
			SWAP(base_pointer, idx + 2, idx + 1, width);
		}

		// should the second be moved to the first?
		if ((*compar_func)(SUB(base_pointer, idx + 1),
				   SUB(base_pointer, idx)) < 0) {
	...
	default:
		...
		if ((thread_count == 0) && (threads_avail > 0) &&
		    (idx > SLICE_THRESH)) {
			threads_avail--;
...
//*/
void quicksort(void *base_pointer, size_t number_of_elements, size_t width,
	       int (*compar_func)(const void *, const void *))
{
  ...
  //*/
	if (number_of_processors == -1) {
		...
		// lwp for each cpu
		if ((number_of_processors > 1) &&
		    (pthread_getconcurrency() < number_of_processors))
			pthread_setconcurrency(number_of_processors);

		// thread count not to exceed THR_PER_LWP per lwp
		threads_avail = (number_of_processors == 1) ?
					0 :
					(number_of_processors * THR_PER_LWP);
	}
