...
int callMemchr(char* pText, char* pPattern, const unsigned int TEXT_SIZE, char** pResult)
{
  printf("\t*** callMemchr() ***\n");

  // init
  if(pText == NULL) return -1;
  if(pPattern == NULL) return -1;
...
int callMemcmp(const char* pText1, const char* pText2, const unsigned int TEXT_SIZE, int* iResult)
{
  printf("\t*** callMemcmp() ***\n");

  // init
  if(iResult == NULL) return -1;
...
// only the content changes, therefore char* are sufficient, since the result changes address, a char** is necessary!
int callMemcpy(char* pText1, const char* pText2, int TEXT_SIZE, char** pResult)
{
  printf("\t*** callMemcpy() ***\n");

  if(pText1 == NULL) return -1;
  if(pText2 == NULL) return -1;
...
// same as memcpy() but works for overlapping arrays (though the second array will be modified!)
int callMemmove(char* pText1, char* pText2, int TEXT_SIZE, char** pResult)
{
  printf("\t*** callMemmove() ***\n"); 

  if(pText1 == NULL) return -1;
  if(pText2 == NULL) return -2;
...
int callMemset(char* pText, int ch, const unsigned int TEXT_SIZE, char** pResult)
{
  printf("\t*** callMemset() ***\n");
...
int main(int argc, char** argv)
{
  ...
  printf("Memfoo() - init\n\n");
  char* text1 = NULL;
  if( (text1 = malloc(sizeof(char) * (strlen(TEXT1) + 1))) == NULL) return -1;
  strcpy(text1, TEXT1);
  const unsigned int TEXT1_SIZE = (strlen(text1) + 1);

  char* text2 = NULL;
  if( (text2 = malloc(sizeof(char) * (strlen(TEXT2) + 1))) == NULL) return -1;
  strcpy(text2, TEXT2);
  const unsigned int TEXT2_SIZE = (strlen(text2) + 1);

  char* text3 = NULL;
  if( (text3 = malloc(sizeof(char) * (strlen(TEXT3) + 1))) == NULL) return -1;
  strcpy(text3, TEXT3);
  ...
  /*
    memchr() - searches the first occurance of a character "ch" in "count" characters in "buffer"
    returns a pointer on this first occurance or NULL ptr
  //*/
  ...
  /*
    ...
    in case of overlapping arrays, memmove() the array "to" will contain the correct output, but "from" will be modified
  ...
  /*
    memset() - copies the low-order-byte of "ch" into the first "count" characters of the array "buf"
    the low-order-byte is the eight bits in a 16 bit word, farthest to the right
    is used to initialize an array to '0' or '\0'
