...
int tracer(pid_t pid)
{
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (!stepwise) {
        /*
                                        // breakpoint - restore orig
        instructions int last_sig = 0; ptrace(PTRACE_SINGLESTEP, pid, 0,
        last_sig); waitpid(pid, &status, 0);
        ...
        0xffff;
        //						return (event ==
        PTRACE_EVENT_EXIT) ? 0 : 1; if (event == PTRACE_EVENT_EXIT) { continue;
                                                        }

        //			do {
                                                        ret =
        ptrace(PTRACE_POKEUSER, pid, sizeof(long)*REGISTER_IP, bp->code_addr);
        //			} while (ret == -1L && (errno == EBUSY || errno
        == EFAULT || errno == ESRCH));
...
pid_t fork_inferior(char **prog, void (*traceme_fun)(void))
{
  ...
  sizeof(*identifier)))){ perror("calloc() failed"); exit(EXIT_FAILURE);
          }
...
static void inf_ptrace_create_inferior(char **prog) // omitting further params
{
