...
int init_text(char** pResult, char* pText, unsigned int* pSize)
{
  ...
  *pSize = (strlen(pText) + 1);
  if( (*pResult = calloc(*pSize, sizeof(**pResult))) == NULL) return -1;
...
{
  if( (*pResult = malloc(sizeof(**pResult))) == NULL) return -1;
  **pResult = value;
...
/*
  allocates memory and copies into a new char*, if it works out, it changes the old size value to the new value
...
int trim_char(char** pStr, const unsigned int COPY_SIZE, unsigned int *pOld_size, const unsigned int NEW_SIZE)
{
  ...
  if( (pNewStr = calloc(NEW_SIZE, sizeof(*pNewStr))) == NULL) return -1;
  if( (pNewStr = strncpy(pNewStr, *pStr, COPY_SIZE)) == NULL){
    if(pNewStr != NULL) free(pNewStr);
    return -1;
  }
  ...
  *pStr = pNewStr;
  ...
  // check if '\0' is set
  if((*pStr)[NEW_SIZE-1] != '\0') (*pStr)[NEW_SIZE-1] = '\0';
