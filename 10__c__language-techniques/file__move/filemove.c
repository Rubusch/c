...
#include <errno.h> /* errno evaluation */
 ...
 * move files
 *
 * - errors, if file is missing (errno)
 * - destination present, will be overwritten
 * - rename() fails for moving to mounted device (cross-link)
 */
...
int main(int argc, char **argv)
{
  ...
  if (1 == argc) {
    printf("Usage: %s <filename>, e.g.: %s text.txt\n", argv[0], argv[0]);
    return EXIT_FAILURE;
  }
  strncpy(file, argv[1], FILENAME_MAX-1);

  // get a random number
  srand(time(NULL));
  int rn = rand();

  // move a file (generated by Makefile), depending if the random number is
  // even/odd to one of those names
  if (file_move(file, (rn%2) ? "Mickey-Mouse.txt" : "Donald-Duck.txt")) {
    printf("Oh oh! %s\n", strerror(errno));
    return EXIT_FAILURE;
  }
