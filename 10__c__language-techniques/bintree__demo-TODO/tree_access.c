...
/*
  ...
  refreshes the number of elements
//*/
int populate_tree(leaf* first, unsigned int* elements, const unsigned int number)
{
  if(NULL == first){
  ...
  for(idx=0; (number > cnt_new_elements) && (MAX_NODES > number) && (MAX_NODES > idx); ++idx){
    ...
    if(0 > init_node(&lf, data)){
      fprintf(stderr, "ERROR: populate_tree() - failed due to init of node.\n");
      return -1;
    }
...
int delete_element(leaf* first, unsigned int* number, const unsigned int data)
{
  ...
  if(NULL == number){
    fprintf(stderr, "ERROR: delete_elemente() - number of elements is NULL\n");
    return -1;
  }
  ...
  leaf* dummy = first;
  if(first->data == data){
    fprintf(stderr, "ERROR: delete_element() - element was root node\n");
    return -1;
    ...
    fprintf(stderr, "ERROR: delete_elemente() - element to delete not found\n");
    return -1;
  }
...
int get_max_level(leaf* node, unsigned int level)
{
  ...
  if(result_level > MAX_LEVEL) return MAX_LEVEL;
  return result_level;
}
...
//*/
int data_to_string(char* str, const unsigned int str_size, const unsigned int data)
{
  //  puts("\tdata_to_string()");
...
int get_data_by_path(leaf* first, char* data, const unsigned int data_len
  ...
  for(idx_path=0; (idx_path <= idx_col/2) && (NULL != element); ++idx_path){
    if(path[idx_path] == '1'){
...
int write_tree(leaf* first)
{
  ...
  const unsigned int line_size = (idx_col_max / 2 + 1) * data_len + ((idx_col_max / 2 + 1) -1) + 2;
  ...
  for(idx_row = 0; idx_row <= idx_row_max; ++idx_row){
    ...
    // set up mask
    if(0 > get_paint_mask_per_row(paint_curr, idx_col_max+2, idx_row_max, idx_row)){
