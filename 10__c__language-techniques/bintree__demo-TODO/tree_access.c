                  ...
                  const unsigned int number)
{
  ...
  for (idx = 0;
       (number > cnt_new_elements) && (MAX_NODES > number) && (MAX_NODES > idx);
       ++idx) {
    lf = NULL;
...
int write_tree(leaf *first)
{
  ...
  const unsigned int line_size =
      (idx_col_max / 2 + 1) * data_len + ((idx_col_max / 2 + 1) - 1) + 2;
  ...
  for (idx_row = 0; idx_row <= idx_row_max; ++idx_row) {
    ...
    // set up mask
    if (0 > get_paint_mask_per_row(paint_curr, idx_col_max + 2, idx_row_max,
                                   idx_row)) {
    ...
    for (idx_col = 0; (idx_col <= idx_col_max); ++idx_col) {
      ...
      if (idx_col % 2 == 0) {
        if (paint_curr[idx_col] == '1') {
          // row to write data
          get_data_by_path(first, data, data_len, path, idx_col_max / 2 + 1,
                           idx_col);
