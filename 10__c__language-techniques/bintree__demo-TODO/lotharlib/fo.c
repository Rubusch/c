...
int get_write_file_pointer(FILE **fp, char filename[FILENAME_MAX])
{
  ...
  ***/
                     ...
                     const unsigned long int CONTENT_SIZE)
  {
    ...
    while ((idxLine < BUFSIZ) && (idxContent < CONTENT_SIZE) &&
      ...
      if (idxLine >= BUFSIZ) {
        fprintf(stderr, "fo::write_linewise(FILE*, char*, const unsigned long) "
                        "- Failed!\n");
        return -1;
      }
  ...
  ***/
  ...
  //*/
  unsigned int number_of_tempfiles() { return TMP_MAX; }
  ...
  //*/
  // CHECKED - OK
  unsigned int number_of_characters_in_static_temp() { return L_tmpnam; }
