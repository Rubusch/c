...
int main(int argc, char **argv)
{
	...
	for (round = 0; round < nrounds; round++) {
		...
		fprintf(stdout,
			"TEST: dequeue__append(content) - NULL != dequeue__first()");
		...
		fprintf(stdout,
			"TEST: dequeue__append(content) - NULL != dequeue__last()");
		...
		fprintf(stdout,
			"TEST: dequeue__remove(content) - NULL == dequeue__first()");
		...
		fprintf(stdout,
			"TEST: dequeue__remove(content) - NULL == dequeue__last()");
		item_b = dequeue__last();
		assert(NULL == item_b);
		assert(item_a == item_b);
		fprintf(stdout, " - OK\n");
		...
		fprintf(stdout, "TEST: %d == dequeue__size()",
			NUMBER_OF_ELEMENTS);
		res = dequeue__size();
		...
		if (NULL != dequeue__first()) {
			cnt = 1;
			for (item = dequeue__first(); item != dequeue__last();
		...
		fprintf(stdout,
			"TEST: NULL == dequeue__prev(dequeue__first())");
		...
		while (dequeue__size()) {
			fprintf(stdout, "TEST: dequeue__remove(first) - '%s'",
				dequeue__first()->content->msg);
			item = dequeue__first();
