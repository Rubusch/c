...
void generateTree(char content[]) { root = getNewItem(content); }
...
static char *getContent(item *node) { return node->content; }


static item *getLeftItem(item *node) { return node->lhs; }


static item *getRightItem(item *node) { return node->rhs; }
...
void addItemSorted(char content[])
{
  ...
  while (!bAdded) {
    ...
    while ((cnt < strlen(content)) && (cnt < strlen(getContent(pItem)))) {
      if ((cnt < strlen(content) - 1) &&
          (cnt < strlen(getContent(pItem)) - 1) &&
...
#ifdef DEBUG
        printf("content (\'%c\'=%i of \'%s\') < pItem->content (\'%c\'=%i of "
               "\'%s\')\t\t/\n",
               content[cnt], content[cnt], content, getContent(pItem)[cnt],
               getContent(pItem)[cnt], getContent(pItem)); // XXX
#endif
      ...
      } else { // strings are equal or at least equal to the first part of the
               // other string
#ifdef DEBUG
        printf("ELSE - strings \'%s\' and \'%s\' are equal\n", content,
               getContent(pItem)); // XXX
#endif
