// shakersort.c
/*
  Shaker sort is in-place (doesn't need further ressources proportional to the amount of 
  elements to sort).

  Shaker sort is stable (an alphabetic order of elements, equal by another key, e.g. birth 
  dates sorted another time by that second key, remains in the same alphabetic order). 
...
void sort(void** first)
{
  ...
  if((NULL == getnextelement(*first)) && (NULL == getprevelement(*first))){
    return;
  }
