// quicksort.c
/*
  Classical recursive quicksort implementation running on a queue. Invoking threads on the single steps might give an 
  enhancement performance tremendously.

  Using quicksort rather on an array like structure seems to be faster since the elements can be indexed directly - an 
  intermediate layer here for a linked list / queue might be the implementation of an iterator, to provide a similar 
  mechanism easily.

  Quicksort can be implemented iterative or recursive, though it's a classic example of recursiveness. 

  This is a recursive implementation of a quicksort using the first element as pivot element.
//*/
...
{
  if(NULL == elem1) return -1;
  if(NULL == elem2) return -1;
  ...
  while(1){
    ...
    // setting environment
    if(b_next) b_next->prev = ptr_a;
    if(b_prev) b_prev->next = ptr_a;
    if(a_next) a_next->prev = ptr_b;
    if(a_prev) a_prev->next = ptr_b;
...
/*
  swap neighbor elements - elem_a MUST be the lower (prev-er / lefter) one, elem_b the higher (next-er / righter) one!!
//*/
