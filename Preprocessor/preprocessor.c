...
// 2. for debugging:
#define DEBUG
// if the line above is commented out, the printf below (in #ifdef... and #endif) 
// won't be compiled -> no output, try it!
...
// 4. define a constant value, that can e.g. also be used for fixed sizes of static arrays
#define ARRAYSIZE 2
#define TRUE "certo!"
#define FALSE "errado!"
...
/*
  ...
  the preprocessor basically tells the compiler which parts of the code to translate.
  - all preprocessor directives start with '#'
  - using #pragma you can set compiler specific options for the compilation
  - preprocessor directives tell the compiler where to look for code to include (e.g. #include <xyz.h>)
  - preprocessor directives tell the compiler to replace defined tags by other code (macros e.g. #define)
  - preprocessor directives tell the compiler to translate only certain partes of the code file xyz.c (e.g. #ifdef, #elif, #endif)
