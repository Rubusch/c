...
static item *getNewItem(char content[])
{
	...
	if (pItem == NULL) {
		fprintf(stderr,
			"bintree_item::getNewItem() - Bad allocation!\n");
		exit(8);
	}
...
void addItemSorted(char content[])
{
	...
	while (!bAdded) {
		cnt = 0;
		while ((cnt < strlen(content)) &&
		       (cnt < strlen(getContent(pItem)))) {
