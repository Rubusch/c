...
int main(int argc, char** argv)
{
  ...
  case 0:
    ...
    do_semctl(sem_id, 0, sem_cmd, sem_un);
    {
      ...
      fprintf(stderr, "the values of the %ld semaphores are:\n", sem_id_ds.sem_nsems);
  ...
  case 1:
    puts("GETNCNT - get number of processes waiting for semaphore value to increas");
    sem_cmd = GETNCNT;
    // xxx
    readnumber(&sem_num, SEM_NUM_DIGITS, "enter a semaphore number");
    // xxx
    sem_un.val = 0;
  ...
  case 2:
    puts("GETPID - get PID of last process to successfully complete a semctl(SETVAL), semctl(SETALL), or semop() on the semaphore");
    sem_cmd = GETPID;
    // xxx
    sem_un.val = 0;
    do_semctl(sem_id, 0, sem_cmd, sem_un);
  ...
  case 4:
    puts("GETZCNT - get number of processes waiting for semaphore value to become zero");
    sem_cmd = GETZCNT;
    // xxx
    readnumber(&sem_num, SEM_NUM_DIGITS, "enter a semaphore number");
    // xxx
  ...
  case 6:
    ...
    do_stat();
    readnumber(&sem_id_ds.sem_perm.uid, SEM_PERM_UID_DIGITS, "enter sem_perm.uid value:");
    readnumber(&sem_id_ds.sem_perm.gid, SEM_PERM_GID_DIGITS, "enter sem_perm.gid value:");
    readnumber((unsigned int*) &sem_id_ds.sem_perm.mode, SEM_PERM_MODE_DIGITS, "enter sem_perm.mode value:");
    do_semctl(sem_id, 0, sem_cmd, sem_un);
  ...
  case 8:
	...
	readnumber((unsigned int*) &sem_un.array[idx], SEM_UN_ARRAY_DIGITS, "value:");
      }
    }
    do_semctl(sem_id, 0, sem_cmd, sem_un);
    // xxx
  ...
  case 9:
    ...
    readnumber(&sem_num, SEM_NUM_DIGITS, "enter a semaphore number");
    // xxx
    readnumber(&sem_un.val, SEM_UN_VAL_DIGITS, "enter a semaphore vlaue");
    do_semctl(sem_id, sem_num, sem_cmd, sem_un);
    // xxx
    sem_un.val = 0;
  ...
  puts("READY.");
  if(NULL != sem_un.array) free(sem_un.array); // TODO check
  exit(EXIT_SUCCESS);
}
...
static void do_semctl(int sem_id, int sem_num, int sem_cmd, union semun sem_un)
{
  ...
  case SETALL:
    ...
    for(idx=0; idx<sem_id_ds.sem_nsems;){
      fprintf(stderr, "%d", sem_un.array[++idx]);
      if(idx < sem_id_ds.sem_nsems) fprintf(stderr, ", ");
    }
    fprintf(stderr, "])\n");
    break;
  default:
...
static void do_stat()
{
  ...
  fprintf(stderr, "sem_otime = %s", sem_id_ds.sem_otime ? ctime(&sem_id_ds.sem_otime) : "not set\n");
