...
int main(int argc, char** argv)
{
  ...
  if(0 > (pid = fork())){
	...
	fprintf(stderr, "\n\tchild: sem_ops[%d]..sem_num = %d, ", idx, sem_ops[idx].sem_num);
	...
	fprintf(stderr, "child process taking control of the track: %d/3 times\n", i+1);
	...
	if(0 > (idx = semop(sem_id, sem_ops, nsem_ops))){
	  ...
	  // halt process for 5 seconds to allow parent to catch semaphor change first
	  sleep(5); 
	}
	++i;
      }
    ...
    while(i < 3){
      ...
      for(idx=0; idx<nsem_ops; ++idx){
	fprintf(stderr, "\tparent: sem_ops[%d].sem_num = %d, ", idx, sem_ops[idx].sem_num);
	fprintf(stderr, "sem_op = %d, ", sem_ops[idx].sem_op);
