...
struct hostent {
  ...
  char **h_addr_list; // array of pointers to addresses for the corresponding
                      // computers
};
/*
...
/*
  ...
  int type                             // socket type
      ...
      SOCK_RAW       Raw Socket
      ...
  int protocol                         // lots of options and some errors, see
"man 2 socket"
...
/*
  ...
  struct sockaddr *addr               // pointer to the target structur to
connect the socket socklen_t addrLen                   // length of the address
in byte (int?)
...
/*
  int sockfd                          // socket descriptor
  int backlog                         // maximum number of possible tries to
connect, 5 is common

  errors:
  EADDRINUSE                          // another socket already listening on
...
/*
  ...
  struct sockaddr* peer               // pointer on the already allocated memory
...
/*
  int nfds                            // highest numbered file descriptor plus
one fd_set* readfds                     // read file descriptor fd_set* writefds
// write file descriptor fd_set* exceptfds                   // exeception file
descriptor (exceptions) struct timeval* timeout             // timeout (secs and
msecs) struct timespec* timeout            // timeout (secs and nsecs) struct
sigset_t*sigmask             // sigmask

  errors:
  EBADF                               // file descriptor invalid or already
closed EINTR                               // was interrupted by a signal EINVAL
// file descriptor negativ or timeout invalid ENOMEM // unable to allocate
memory
...
if (sendto(s, text, strlen(text), 0, ( struct sockaddr * )&addr,
           sizeof(addr)) == -1) {
...
/*
  ...
  SHUT_RDWR                            // further receptions and transmissions
disallowed
