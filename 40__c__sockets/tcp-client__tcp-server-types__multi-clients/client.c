...
#define NBYTES                                                                 \
	24 /* number of max bytes to read,
...
int lothars__tcp_connect(const char *host, const char *serv)
{
	...
	do {
		if (0 > (sockfd = socket(res->ai_family, res->ai_socktype,
					 res->ai_protocol))) {
...
{
	int idx, jdx, fd_connect, n_children, n_loops, n_bytes;
	...
	for (idx = 0; idx < n_children; ++idx) {
		...
		if (0 == (pid = lothars__fork())) {
			...
			for (jdx = 0; jdx < n_loops; ++jdx) {
				...
				fd_connect =
					lothars__tcp_connect(serverip, port);

				// write to socket
				lothars__write(fd_connect, request,
					       strlen(request));

				// read response
				if (n_bytes !=
				    (bytes = lothars__readn(fd_connect, reply,
							    n_bytes))) {
