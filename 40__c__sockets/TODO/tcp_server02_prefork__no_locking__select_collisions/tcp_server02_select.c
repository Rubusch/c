// tcp_server02_select.c
/*
  resources:
  serv02l.c; UNIX Network Programming, p. XXX


  prefork, no locking, block in select instead of accept to see select
  collisions; works on BSD-derived systems but not on SVR4.
...
pid_t child_make( int idx, int listenfd, socklen_t clilen )
{
        ...
        do {
                FD_SET( listenfd, &rset ); // select - register listenfd

                // select - call, BLOCKING
                /*
                  socket blocks here, when the socket becomes "ready for r/w"
                  select gives the way free, and an incoming connection can
                  connect with the accept(), waiting further below

                  having an incomning connection triggering the select() of
                  several children, they will give the way free for the blocking
                  accept()

                  [test by starting server and connecting only once, see the
                  output]

                  select makes active looping (polling) obsolete
                 */
                if (0 > (nready = select( listenfd + 1
                                          , &rset
                                          , NULL
                                          , NULL
                                          , NULL
                ...
                if (0 == FD_ISSET( listenfd, &rset )) { // select - listenfd ready for read and write
                        perror( "listenfd readable" );
                        kill( getpid(), SIGINT );
                }
                fprintf( stderr, "\t%s:%d - select, listenfd ready for read and write\n", __FILE__, __LINE__ );
