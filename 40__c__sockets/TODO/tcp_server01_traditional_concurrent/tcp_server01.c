...
/*
  resources:
  serv01.c; UNIX Network Programming, p. XXX


  using fork(), and corresponding signaling
  one fork per client (traditional concurrent server)
...
void sig_chld( int signo )
{
        pid_t pid;
        int stat;
        fprintf( stderr, "\t%s:%d - SIGCHLD caught\n", __FILE__, __LINE__ );
...
int main( int argc, char* argv[] )
{
        ...
        */
        ...
        if ((SIG_ERR == setup_signal( SIGCHLD, sig_chld ))
            || (SIG_ERR == setup_signal( SIGINT, sig_int ))) {
                perror( "signal error" );
