...
char *sock_ntop_host(const struct sockaddr *sa, socklen_t salen)
{
	...
	case PF_INET: {
		...
		if (NULL ==
		    inet_ntop(PF_INET, &sin->sin_addr, str, sizeof(str))) {
			return NULL;
		}
		return str;
	}
	...
	case PF_INET6: {
		...
		if (NULL ==
		    inet_ntop(PF_INET6, &sin6->sin6_addr, str, sizeof(str))) {
			return NULL;
		}
		return str;
	}
...
int main(int argc, char **argv)
{
	...
	for (ifihead = ifi = lothars__get_ifi_info(family, doaliases);
	     ifi != NULL; ifi = ifi->ifi_next) {
		fprintf(stdout, "%s: ", ifi->ifi_name);

		if (ifi->ifi_index != 0)
			fprintf(stdout, "(%d) ", ifi->ifi_index);
		fprintf(stdout, "< ");
