...
int main(int argc, char **argv)
{
    ...
    //*/
      ...
      //*/
			...
			fprintf(stderr,
				"child calling semop(%d, &sem_ops, %d)\n",
				sem_id, nsem_ops);
			for (idx = 0; idx < nsem_ops; ++idx) {
				fprintf(stderr,
					"\n\tchild: sem_ops[%d]..sem_num = %d, ",
					idx, sem_ops[idx].sem_num);
			...
			if (-1 == (idx = semop(sem_id, sem_ops, nsem_ops))) {
				...
				if (0 >
				    (idx = semop(sem_id, sem_ops, nsem_ops))) {
					perror("child: semop failed");
				} else {
					fprintf(stderr,
    ...
    //*/
		...
		while (i < 3) {
			...
			/* recap the call to be made //*/
			fprintf(stderr,
				"parent caling semop(%d, &sem_ops, %d) with:\n",
				sem_id, nsem_ops);
			for (idx = 0; idx < nsem_ops; ++idx) {
				fprintf(stderr,
					"\tparent: sem_ops[%d].sem_num = %d, ",
					idx, sem_ops[idx].sem_num);
				fprintf(stderr, "sem_op = %d, ",
					sem_ops[idx].sem_op);
