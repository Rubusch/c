// pthread_mutex.c
/*
  MUTEXES (used generally for synchronizing two threads, can provoke deadlocks with more threads)
...
void* dotprod(void* arg)
{
  ...
  /*
    lock the mutex prior to updating the value in the shared structure and unlock it upon 
    updating
  //*/
...
int main(int argc, char** argv)
{
  ...
  if(NULL == (aVal = (double*) malloc(NUMTHRDS*VECLEN*sizeof(*aVal)))){
    perror("malloc() failed");
    exit(1);
  }
  if(NULL == (bVal = (double*) malloc(NUMTHRDS*VECLEN*sizeof(*bVal)))){
    perror("malloc() failed");
    exit(1);
  }
