...
int main(int argc, char **argv)
{
  ...
  /*
    readnumber(&max_msg_sz, TMP_SIZE, "enter the message buffer size you
  want:");
  //*/
    ...
    case 1:
      ...
      { // set IPC_NOWAIT flag
        ...
        do {
          ...
          if ((strlen("no") == (strlen(tmp) - 1)) ||
              (strlen("yes") == (strlen(tmp) - 1))) {
...
void readlongnumber(signed long *val, const unsigned int digits,
                    const char *comment)
{
  if (NULL == comment) {
    perror("text is NULL");
